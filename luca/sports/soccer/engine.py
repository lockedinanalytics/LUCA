...
class SoccerEngine(SportEngine):
    ...
    def evaluate_games(self, games: List[TeamGame], markets: List[MarketLine]) -> LucaRunResult:
        ...
        return LucaRunResult(sport=self.sport, league="Soccer", date=games[0].date if games else "", slate_size=len(games), games_evaluated=len(games), data_completeness=.80 if games else 0, evaluations=evaluations, recommendations=recommendations, pass_list=pass_list)
