...
class MlbPitcherAdapter:
    ...
    def build(self, pitcher_id: str | int | None) -> StartingPitcherInput:
        ...
        return StartingPitcherInput(
            xera=era,
            fip=fip if fip is not None else self._estimate_fip(stat),
            strikeout_rate=strikeout_rate,
            walk_rate=walk_rate,
            hard_hit_rate=self._to_float(stat.get("hardHitRate")),
            barrel_rate=self._to_float(stat.get("barrelRate")),
